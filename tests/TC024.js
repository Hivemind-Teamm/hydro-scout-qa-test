...
async function TC024() {
    ...
    try {
        console.log("🚀 Starting Test Case 024: Info panel close and fly-to...");
        ...
        let hydrantName = await driver.wait(until.elementLocated(By.xpath("//button[@title='Zoom to hydrant']")), 10000);
        await hydrantName.click();
        await driver.findElement(By.css("button[aria-label='Close']")).click();
        await driver.sleep(1000);
        let panel = await driver.findElements(By.xpath("//button[contains(., 'Open full details')]"));
        if (panel.length === 0) {
            console.log("🏆 TEST PASSED: Fly-to action worked and info panel closed.");
        } else {
            console.log("❌ TEST FAILED: Info panel did not close.");
        }
    } catch (error) {
