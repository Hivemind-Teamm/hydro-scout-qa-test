...
async function TC022() {
    ...
    try {
        console.log("🚀 Starting Test Case 022: Location search no results...");
        ...
        await search.sendKeys('zzzzzzzzzzzz-no-place-12345');
        await driver.sleep(3000);
        let body = await driver.findElement(By.css('body'));
        if ((await body.getText()).length > 0) {
            console.log("🏆 TEST PASSED: No-results search did not crash the dashboard.");
        } else {
            console.log("❌ TEST FAILED: Page became blank after failed search.");
        }
    } catch (error) {
