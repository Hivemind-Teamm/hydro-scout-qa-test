...
// Zooms in to isolate the marker near canvas center, then clicks it. The
// Full Details side panel resizes the map container when opened/closed, which
// shifts pixel<->geo mapping - so this should be re-run fresh (ideally after
// a page reload) any time we need to reselect following a Full Details visit.
...
async function TC037() {
    ...
    let addressTag = `SELENIUM-TC037-${Date.now()}`;
    let hydrantCreated = false;

    try {
        console.log("🚀 Starting Test Case 037: Resolve or deny report...");
        ...
        // --- File a damage report BEFORE ever opening Full Details, since
        // opening it resizes the map and invalidates marker screen positions ---
        ...
        await description.sendKeys('Selenium automated resolve-report test');
        ...
        await driver.sleep(2500);
        console.log("Disposable damage report filed.");

        // Now verify the hydrant identity via Full Details (report is already filed).
        ...
        if (tagMatch.length === 0) {
            throw new Error("Could not confirm the disposable hydrant identity via its landmark tag.");
        }
        ...
        console.log("🏆 TEST PASSED (resolve): Disposable report was resolved.");

        // --- Cleanup: delete the report (admin-only). It no longer shows
        // under the Pending filter, so switch to All to find it again. ---
        ...
        // --- Cleanup: decommission the disposable hydrant. Reload fresh first
        // so the marker search starts from a clean, unresized map state. ---
        ...
        if (tagMatch2.length === 0) throw new Error("Lost track of the disposable hydrant before decommission - aborting to avoid deleting the wrong record.");
